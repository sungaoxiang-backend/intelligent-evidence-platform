...
export const TableWithAttrs = Table.extend({
  ...
  addCommands() {
    ...
    return {
      ...parentCommands,
      insertCellAbove: () => ({ tr, state, dispatch }: any) => {
        ...
        if (addRowBefore) {
          return addRowBefore()({ tr, state, dispatch } as any)
        }
        return false
      },

      insertCellBelow: () => ({ tr, state, dispatch }: any) => {
        if (!isInTable(state)) return false

        // 在当前位置下方插入新行
        const addRowAfter = parentCommands.addRowAfter
        if (addRowAfter) {
          return addRowAfter()({ tr, state, dispatch } as any)
        }
        return false
      },

      deleteCell: () => ({ tr, state, dispatch }: any) => {
        if (!isInTable(state)) return false
        ...
        if (colspan > 1 || rowspan > 1) {
          ...
          if (deleteRow) {
            return deleteRow()({ tr, state, dispatch } as any)
          }
          return false
        }
      ...
      splitCell: hasSplitCell ? parentCommands.splitCell : () => ({ tr, state, dispatch }) => {
        ...
        // 找到当前单元格位置
        const cell = findCell(selection.$anchor) as any
...
export const TableRowWithAttrs = TableRow.extend<{ isComplexForm?: boolean }>({
  ...
  renderHTML({ node, HTMLAttributes }) {
    ...
    return [
      "tr",
      mergeAttributes(
        (this.options as any).HTMLAttributes,
        HTMLAttributes,
        dataAttrs,
        styleAttrs
      ),
