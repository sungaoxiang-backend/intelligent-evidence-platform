...
@router.get("/with-users", response_model=ListResponse[CaseWithUser])
async def read_cases_with_users(
    ...
    """获取案件列表，包含用户信息"""
    cases, total = await case_service.get_multi_with_users_with_count(db, skip=skip, limit=limit)
    return ListResponse(
        data=cases,
        pagination=Pagination(total=total, page=skip // limit + 1, size=limit, pages=(total + limit - 1) // limit)
    )
...
@router.get("/{case_id}/with-user", response_model=SingleResponse[CaseWithUser])
...
@router.delete("/{case_id}", response_model=SingleResponse)
